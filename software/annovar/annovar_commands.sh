annotate_variation.pl -buildver hg19 -downdb cytoBand humandb/
annotate_variation.pl -buildver hg19 -downdb refGene humandb/
annotate_variation.pl -buildver hg19 -downdb gwasCatalog humandb/
annotate_variation.pl -buildver hg19 -downdb -webfrom annovar snp129 humandb/
annotate_variation.pl -buildver hg19 -downdb -webfrom annovar 1000goct2014 humandb/
annotate_variation.pl -buildver hg19 -downdb -webfrom annovar clinvar_20160302 humandb/
annotate_variation.pl -buildver hg19 -downdb -webfrom annovar dbnsfp30a humandb/ 
annotate_variation.pl -buildver hg19 -downdb -webfrom annovar exac03 humandb/ 
annotate_variation.pl -buildver hg19 -downdb -webfrom annovar cg69 humandb/
annotate_variation.pl -buildver hg19 -downdb -webfrom annovar esp6500siv_all humandb/
annotate_variation.pl -buildver hg19 -downdb -webfrom annovar cosmic70 humandb/
annotate_variation.pl -buildver hg19 -downdb -webfrom annovar icgc21 humandb/
annotate_variation.pl -buildver hg19 -downdb -webfrom annovar nci60 humandb/